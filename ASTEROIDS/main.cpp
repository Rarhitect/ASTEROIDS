...
public:
  ...
  void draw(RenderWindow &app, int score = 0)
  {
    ...
    sf::Font font;
    if(!font.loadFromFile("/Users/alyaev.roman/Desktop/Y/ASTEROIDS/Resources/images/sansation.ttf"))
    {
      std::cout << "oops!";
    }
    ...
    Interface_score(float x_, float y_, Color color_, RenderWindow & app, int score)
    {
      x = x_;
      y = y_;
      color = color_;
...
public:
  ...
  void draw(RenderWindow &app, int live = 0)
  {
    ...
    Interface_health(float x_, float y_, Color color_, RenderWindow & app, int health)
    {
      x = x_;
      y = y_;
      color = color_;
      name = "interface_health";
      this->draw(app, health);
    }
};
...
int main()
{
  ...
  while (app.isOpen() && (!close))
  {
    ...
    Interface_score insc(W-100,0, Color(175, 180, 240), app, p->get_scores());

    Interface_health inhe(0,0, Color(175, 180, 240), app, p->get_life());
